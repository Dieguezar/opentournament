...
describe.skipIf(!hasDb)('integración de la API (requiere PostgreSQL)', () => {
  ...
  it(
    ...
    async () => {
      ...
      const smashParticipants = await db
        .select({ id: tournamentParticipants.id })
        .from(tournamentParticipants)
        .where(eq(tournamentParticipants.tournamentId, firstSeed.smashTournamentId));
      const smashMatches = await db
        .select({ status: matches.status, result: matches.result })
        .from(matches)
        .where(eq(matches.tournamentId, firstSeed.smashTournamentId));
      const smashGames = smashMatches.flatMap((match) => match.result?.games ?? []);
      ...
      expect(smashParticipants).toHaveLength(8);
      expect(smashMatches).toHaveLength(15);
      expect(smashMatches.every((match) => match.status === 'finalized')).toBe(true);
      expect(smashGames.length).toBeGreaterThan(30);
      expect(new Set(smashGames.map((game) => game.stage)).size).toBeGreaterThanOrEqual(5);
      expect(
        new Set(smashGames.flatMap((game) => [game.homeCharacter, game.awayCharacter])).size,
